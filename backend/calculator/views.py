...
class PetNutritionCalculator(APIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter('conditions', openapi.IN_FORM, type=openapi.TYPE_STRING, required=False, description="慢性病（JSON 字串，如 [\"慢性腎臟病\",\"心臟病\"]），或重複傳表單欄位 conditions"),
        ],
        responses={200: "成功"}
    )
        ...
        try:
            ...
            if language == 'en':
                ...
                message = (
                    ...
                    f"{rec_label}\n" + "\n".join([f"- {k}: {v}g" for k, v in rec.items()]) +
                    f"\n{actual_label}\n" + "\n".join([f"- {k}: {v}g" for k, v in actual.items()]) +
                    "\n請先列出你的計算結果，包含一天飼料建議攝取量、每項營養素建議攝取量、和與飼料提供營養素量對比，若某項營養素的建議攝取量大於或小於飼料提供營養素量的1.5倍，要在那項營養素的欄位告知使用者可能需要更換飼料，若某項飼料提供的營養素量小於零，要在該項目告知使用者要重新確認是否欄位數值有輸錯，另外，若有健康報告，再根據健康報告給出微調建議，然後根據『慢性病/照護重點』與『飲食提醒』給出照護建議。用繁體中文回答。"
                )
